...
def img2windows(img, H_sp, W_sp):
    ...
    """
    B, C, H, W = img.shape
    img_reshape = img.view(B, C, H // H_sp, H_sp, W // W_sp, W_sp) # 1,32,16,5,8,10 1 16.8.5.10.32
...
class Spatial_Attention(nn.Module):
    ...
    def im2win(self, x, H, W):
        ...
        # print(f"hsp:{self.H_sp}, wsp:{self.W_sp}, num_head:{self.num_heads}, x.shape: {x.shape}")
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # print(f"convx.shape:{conv_x.shape}")
...
if __name__ == '__main__':
    ...
    x = torch.randn((2, 3, height, width)).cuda()
    x = model(x)
